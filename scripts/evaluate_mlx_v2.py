...
def main():
    ...
    def evaluate_variant(use_adapter: bool):
        ...
        for record in held_out:
            ...
            response = mlx_lm.generate(model, tokenizer, prompt=prompt, max_tokens=250, verbose=False)
            ...
            # Extract code block
            code_match = re.search(r"```(?:rust)?(.*?)```", response_clean, re.DOTALL)
            fixed_code = code_match.group(1).strip() if code_match else response_clean
            ...
            with tempfile.TemporaryDirectory() as tmpdir:
                ...
                # rustfmt
                subprocess.run(["rustfmt", f_rs], capture_output=True)
        ...
        compiles_count = sum(1 for r in results if r["compiles"])
        latencies = [r["latency_ms"] for r in results]

        summary = {
            "variant": desc,
            "records_evaluated": len(results),
            "compiles": f"{compiles_count}/{len(results)}",
            "latency_ms": {
